...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            let stock_left = self.storage_inventory.read(product);
            self.storage_inventory.write(product, stock_left + new_stock);
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            let stock_left = self.storage_inventory.read(product);
            assert(stock_left >= quantity, 'Not enough stock');
            self.storage_inventory.write(product, stock_left - quantity);
        }
